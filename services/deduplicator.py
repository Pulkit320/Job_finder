...
# Source hierarchy priorities (higher = better)
SOURCE_PRIORITY = {
...
class DeduplicatorService:
    ...
    def deduplicate(self, existing_jobs: List[Job], new_jobs: List[Job]) -> Tuple[List[Job], int]:
        """
        Deduplicates newly scraped jobs against both each other and the existing job database.
        Returns: (merged_all_jobs, newly_added_count)
        """
        ...
        for candidate in new_jobs:
            ...
            # Check fuzzy match against current pool
            is_dup = False
            for idx, existing in enumerate(pool):
                if self._is_duplicate(candidate, existing):
                    ...
                    # If candidate has higher source priority, replace existing
